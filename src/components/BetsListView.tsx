...
const BetsListView: React.FC<Props> = props => {
	...
	return (
		<ImageBackground source={{uri: gameImage}} style={styles.container}>
			<TouchableOpacity
				...
				<View style={styles.flexRawSpaceBetween}>
					<View style={{flexDirection: 'column'}}>
						<Text style={styles.timeWithTeamNameTextTitle}>
							{'STARTS: ' +
								dateTimeConvert(parseFloat(startTime_timestamp)).toUpperCase()}
						</Text>
						{endTime_timestamp && (
							<Text style={styles.timeWithTeamNameTextTitle}>
								{'ENDS: ' +
									dateTimeConvert(parseFloat(endTime_timestamp)).toUpperCase()}
							</Text>
						)}
					</View>
					{item?.betTag && (
				...
				<View style={styles.bottomItemContainer}>
					<View style={styles.tagContainer}>
						{item?.match?.tagDetails?.includes(Strings.HOT.toLowerCase()) && (
							<TagView
